...
Scans Claude Desktop, Claude Code, Cursor, Gemini CLI, Goose, and `./mcp.json`. Sources: `claude-desktop`, `claude-code`, `cursor`, `gemini`, `goose`, `project`. Claude Code discovery includes global `~/.claude.json` servers and project-scoped entries matching the current working directory.
