...
the discretion of the server and subject to the user’s ability to review and decline.MCP clients **MUST**:
...
**Request Association Requirement**Servers **MUST** send server-to-client requests (such as
`roots/list`, `sampling/createMessage`, or `elicitation/create`) only in
association with an originating client request (e.g., during `tools/call`,
`resources/read`, or `prompts/get` processing).Standalone server-initiated requests of these types on independent
...
authorization](/specification/draft/basic/authorization)). Instead, it’s used when the MCP
...
"url": "https://mcp.example.com/ui/set\_api\_key",
...
* For remote MCP servers, user identification **MUST** be derived from credentials acquired via [MCP authorization](/specification/draft/basic/authorization) when possible (e.g. `sub` claim)
...
Authorization with external APIs enabled by URL mode elicitation is separate from [MCP authorization](/specification/draft/basic/authorization). MCP servers **MUST NOT** rely on URL mode elicitation to authorize users for themselves.
####
[​
](#understanding-the-distinction)
Understanding the Distinction
* **MCP Authorization**: Required OAuth flow between the MCP client and MCP server (covered in the [authorization specification](/specification/draft/basic/authorization))
...
* Correct: Rely on [authorization](/specification/draft/basic/authorization) to identify the user
###
[​
](#form-mode-security)
Form Mode Security
