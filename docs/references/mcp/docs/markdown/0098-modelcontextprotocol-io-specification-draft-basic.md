...
* A `resultType` of `"input\_required"` the request is incomplete and more information is needed to process the request. The result contains an `InputRequiredResult` object with additional information needed.
...
[Lifecycle](/specification/draft/basic/lifecycle) for version negotiation rules.
...
On notifications delivered via a [`subscriptions/listen`](/specification/draft/basic/utilities/subscriptions) stream,
