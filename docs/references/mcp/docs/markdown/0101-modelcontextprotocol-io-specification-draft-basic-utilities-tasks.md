...
Capability Negotiation
Requestors **SHOULD** only augment requests with a task if the corresponding capability has been declared by the receiver. Clients discover server `tasks` capabilities via `server/discover`; servers learn client `tasks` capabilities from `clientCapabilities` in each request’s `\_meta`.
...
After a task completes, the operation result is retrieved via [`tasks/result`](/specification/draft/schema#tasks/result). This is distinct from the initial `CreateTaskResult` response, which contains only task data. The result structure matches the original request type (e.g., `CallToolResult` for `tools/call`).
When a task is in `input\_required` status, `tasks/result` returns an [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult) containing `inputRequests` that the client must fulfill via a [`tasks/input\_response`](#providing-input-for-tasks) request. See [multi round-trip requests](/specification/draft/basic/utilities/mrtr#multi-round-trip-requests) for details on the persistent workflow.
...
Task Status Notification
When a task status changes, receivers **MAY** send a [`notifications/tasks/status`](/specification/draft/schema#notifications/tasks/status) notification to inform the requestor of the change. This notification includes the full task state.
**Notification:**
```
`{
"jsonrpc": "2.0",
...
Listing Tasks
To retrieve a list of tasks, requestors can send a [`tasks/list`](/specification/draft/schema#tasks/list) request. This operation supports pagination.
**Request:**
```
`{
"jsonrpc": "2.0",
...
Cancelling Tasks
To explicitly cancel a task, requestors can send a [`tasks/cancel`](/specification/draft/schema#tasks/cancel) request.
**Request:**
```
`{
"jsonrpc": "2.0",
...
###
[​
](#providing-input-for-tasks)
Providing Input for Tasks
When a task reaches the `input\_required` status and `tasks/result` returns an [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult), the requestor provides the required input via a [`tasks/input\_response`](/specification/draft/schema#tasks/input_response) request. This is the task workflow form of [multi round-trip requests](/specification/draft/basic/utilities/mrtr#multi-round-trip-requests).
The `tasks/input\_response` request carries:
* `taskId`: the ID of the task requiring input
* `inputResponses`: an object mapping each key from the `inputRequests` to the response obtained from the client
* `requestState` (optional): the opaque state token from the `InputRequiredResult`, if provided
**Request:**
```
`{
"jsonrpc": "2.0",
...
"taskId": "786512e2-9e0d-44bd-8f29-789f320fe840",
"status": "working",
"statusMessage": "Input received, resuming operation.",
"createdAt": "2025-11-25T10:30:00Z",
...
`
```
After sending `tasks/input\_response`, the requestor **SHOULD** resume polling via `tasks/get`.
Receivers **MUST** reject `tasks/input\_response` requests for tasks that are not in `input\_required` status with error code `-32602` (Invalid params).
##
[​
...
1. From `working`: may move to `input\_required`, `completed`, `failed`, or `cancelled`
2. From `input\_required`: may move to `working`, `completed`, `failed`, or `cancelled`
...
1. When the task receiver has messages for the requestor that are necessary to complete the task, the receiver **SHOULD** move the task to the `input\_required` status.
...
3. When the requestor encounters the `input\_required` status, it **SHOULD** preemptively call `tasks/result`. If the result is an [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult), the requestor **SHOULD** fulfill the `inputRequests` and submit responses via [`tasks/input\_response`](#providing-input-for-tasks).
4. When the receiver receives all required input, the task **SHOULD** transition out of `input\_required` status (typically back to `working`).
###
[​
...
3. When a receiver receives a `tasks/result` request for a task in `input\_required` status, it **MUST** return an [`InputRequiredResult`](/specification/draft/basic/utilities/mrtr#InputRequiredResult) containing the `inputRequests` that the requestor must fulfill via [`tasks/input\_response`](#providing-input-for-tasks).
4. When a receiver receives a `tasks/result` request for a task in `working` status, it **MUST** block the response until the task reaches a terminal status or `input\_required` status.
5. For tasks in a terminal status, receivers **MUST** return from `tasks/result` exactly what the underlying request would have returned, whether that is a successful result or a JSON-RPC error.
###
[​
...
Task Progress Notifications
Task-augmented requests support progress notifications as defined in the [progress](/specification/draft/basic/utilities/progress) specification. The `progressToken` provided in the initial request remains valid throughout the task lifetime.
###
[​
](#task-listing)
Task Listing
...
* `input\_required`: The receiver needs input from the requestor. The requestor **SHOULD** call `tasks/result` to receive input requests, even though the task has not reached a terminal state.
