...
Bedrock & Google Vertex AI](#using-with-amazon-bedrock-&-google-vertex-ai)
section.
##
[​
](#manual-setup)
...
3. **Update prompt inputs**: Replace `direct\_prompt` with `prompt`
4. **Move CLI options**: Convert `max\_turns`, `model`, `custom\_instructions`, etc. to `claude\_args`
###
[​
...
|`direct\_prompt`|`prompt`|
|`override\_prompt`|`prompt` with GitHub variables|
|`custom\_instructions`|`claude\_args: --append-system-prompt`|
|`max\_turns`|`claude\_args: --max-turns`|
|`model`|`claude\_args: --model`|
|`allowed\_tools`|`claude\_args: --allowedTools`|
|`disallowed\_tools`|`claude\_args: --disallowedTools`|
|`claude\_env`|`settings` JSON format|
###
[​
...
Always use GitHub Secrets (for example, `${{ secrets.ANTHROPIC\_API\_KEY }}`) rather than hardcoding API keys directly in your workflow files.
###
[​
...
* Configure appropriate `--max-turns` in `claude\_args` to prevent excessive iterations
...
claude\_args: "--max-turns 5" # Optional CLI arguments
`
```
Key features:
...
* **CLI passthrough** - Any Claude Code CLI argument via `claude\_args`
...
* Create a new secret named `APP\_ID` with your GitHub App’s ID
...
* `ANTHROPIC\_API\_KEY`: Your Claude API key from [console.anthropic.com](https://console.anthropic.com)
...
|`APP\_ID`|Your GitHub App ID (from app settings)|
|`APP\_PRIVATE\_KEY`|The private key you generated for your GitHub App|
```
...
(github.event\_name == 'issues' && contains(github.event.issue.body, '@claude'))
runs-on: ubuntu-latest
env:
...
**Required GitHub secrets:**
|Secret Name|Description|
|`GCP\_WORKLOAD\_IDENTITY\_PROVIDER`|Workload identity provider resource name|
|`GCP\_SERVICE\_ACCOUNT`|Service account email with Vertex AI access|
|`APP\_ID`|Your GitHub App ID (from app settings)|
|`APP\_PRIVATE\_KEY`|The private key you generated for your GitHub App|
```
`name: Claude PR Action
...
VERTEX\_REGION\_CLAUDE\_4\_5\_SONNET: us-east5
`
```
...
|`claude\_args`|CLI arguments passed to Claude Code|No|
|`anthropic\_api\_key`|Claude API key|Yes\*\*|
|`github\_token`|GitHub token for API access|No|
|`trigger\_phrase`|Custom trigger phrase (default: “@claude”)|No|
|`use\_bedrock`|Use Amazon Bedrock instead of Claude API|No|
|`use\_vertex`|Use Google Vertex AI instead of Claude API|No|
...
Pass CLI arguments
The `claude\_args` parameter accepts any Claude Code CLI arguments:
```
`claude\_args: "--max-turns 5 --model claude-sonnet-4-6 --mcp-config /path/to/config.json"
`
```
Common arguments:
