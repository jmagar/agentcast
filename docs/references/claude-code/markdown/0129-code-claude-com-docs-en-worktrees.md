...
`
```
You can also ask Claude to “work in a worktree” during a session, and it will create one with the [`EnterWorktree`](/docs/en/tools-reference) tool.
...
Subagent worktrees orphaned by a crash or interrupted run are removed at startup once they are older than your [`cleanupPeriodDays`](/docs/en/settings#available-settings) setting, provided they have no uncommitted changes, no untracked files, and no unpushed commits. Worktrees you create with `--worktree` are never removed by this sweep.
##
[​
