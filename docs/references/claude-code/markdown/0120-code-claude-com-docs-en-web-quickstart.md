...
* Name the file or function: “Add a README with setup instructions” or “Fix the failing auth test in `tests/test\_auth.py`” is better than “fix tests”
...
|`prompt\_url`|URL to fetch the prompt text from, for prompts too long to embed in a query string. The URL must allow cross-origin requests. Ignored when `prompt` is also set.|
