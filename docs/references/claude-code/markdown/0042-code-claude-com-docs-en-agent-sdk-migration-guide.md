...
```
**3. Update your imports:**
Change all imports from `claude\_code\_sdk` to `claude\_agent\_sdk`:
```
`# Before
...
from claude\_code\_sdk import query, ClaudeCodeOptions
options = ClaudeCodeOptions(model="claude-opus-4-7", permission\_mode="acceptEdits")
# AFTER (claude-agent-sdk)
from claude\_agent\_sdk import query, ClaudeAgentOptions
options = ClaudeAgentOptions(model="claude-opus-4-7", permission\_mode="acceptEdits")
`
```
...
prompt: "Hello",
options: {
systemPrompt: { type: "preset", preset: "claude\_code" }
}
});
...
**For Python:**
1. Check that all imports are updated to use `claude\_agent\_sdk`
