...
```
`gh api repos/OWNER/REPO/check-runs/CHECK\_RUN\_ID \\
--jq '.output.text | split("bughunter-severity: ")[1] | split(" --\>")[0] | fromjson'
`
```
This returns a JSON object with counts per severity, for example `{"normal": 2, "nit": 1, "pre\_existing": 0}`. The `normal` key holds the count of Important findings; a non-zero value means Claude found at least one bug worth fixing before merge.
###
[​
