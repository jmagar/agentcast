...
`
```
This saves it to `\~/.claude/channels/telegram/.env`. You can also set `TELEGRAM\_BOT\_TOKEN` in your shell environment before launching Claude Code.
4
[
](#)
...
`
```
This saves it to `\~/.claude/channels/discord/.env`. You can also set `DISCORD\_BOT\_TOKEN` in your shell environment before launching Claude Code.
6
[
](#)
Restart with channels enabled
