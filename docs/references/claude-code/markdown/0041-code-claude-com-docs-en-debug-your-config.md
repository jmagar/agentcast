...
Test against a clean configuration
If targeted checks don’t isolate the cause, or your configuration is in an unknown state, compare against a session that loads nothing from your usual setup. Point [`CLAUDE\_CONFIG\_DIR`](/docs/en/env-vars) at an empty directory to bypass everything under `\~/.claude`, and launch from a directory that has no `.claude` folder, `.mcp.json`, or `CLAUDE.md` so project configuration is also skipped.
```
`cd /tmp && CLAUDE\_CONFIG\_DIR=/tmp/claude-clean claude
`
```
