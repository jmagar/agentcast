...
| `agent-gateway.md` | Partially complete with catalog/routing/search documents, catalog diff metadata, gateway health aggregation, exposure allow/deny policy, protected route config/index/CRUD/status/test policy, OAuth orchestration, metadata discovery, dynamic client registration, authorization-code/refresh-token endpoint exchange, refresh lock integration through the API surface, and removed-upstream OAuth reconciliation. | Reload orchestration only when runtime/config surfaces need live replacement behavior. |
| `agent-search.md` | Partially complete with deterministic action indexing/search, secret-like field redaction, bounded/truncated fields, schema summaries, stable matched-term/matched-field explanation metadata, and gateway API/CLI projection of search metadata. | Extend only when additional ranking signals or UI/API consumers require broader query behavior. |
